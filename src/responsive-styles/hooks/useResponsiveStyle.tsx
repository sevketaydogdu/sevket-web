...
const buildCustomStyleMap = (styles: StyleSheet.NamedStyles<any>, size: DEVICE_SIZES) => {
  ...
  defaultKeys.forEach((key) => {
    ...
    const styleOverrides = Object.keys(styles).filter((style) =>
      style.match(matchOverrides(key, size))
    );
...
export default function useResponsiveStyle<Styles extends StyleSheet.NamedStyles<Styles>>(
  styles: Styles
) {
